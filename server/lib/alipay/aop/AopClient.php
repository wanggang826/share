...
class AopClient {
    ...
    function __construct($appid, $gateway, $privateKey, $publicKey, $publicKeyOfAlipay, $apiVer, $signType) {
        $this->appId = $appid;
        $this->gatewayUrl= $gateway;
        $this->rsaPrivateKeyFilePath = $privateKey;
        $this->rsaPublicKeyFilePath = $publicKey;
        $this->alipayPublicKey = $publicKeyOfAlipay;
	...
	*/
	...
	public function execute($request, $authToken = null, $appInfoAuthtoken = null, $getRequestUrl=false) {
		$this->setupCharsets($request);
        ...
        if($getRequestUrl) {
            ...
            foreach ($apiParams as $apiParamKey => $apiParamValue) {
                $requestUrl .= "$apiParamKey=" . urlencode($this->characet($apiParamValue, $this->postCharset)) . "&";
            }
            $requestUrl = substr($requestUrl, 0, -1);
            return $requestUrl;
        }
		...
		try {
            ...
            LOG::ERROR('Exception AliPay API, method: ' . $sysParams['method'] . ', requestUrl: ' . $requestUrl . ', $e code: '. $e->getCode() . ', $e message: ' . $e->getMessage());
	...
	public function rsaCheckV1($params, $rsaPublicKeyFilePath) {
		...
		$params['sign'] = null;
		return $this->verify($this->getSignContent($params), $sign, $rsaPublicKeyFilePath, $this->signType);
	}
	...
	public function verifygw($is_sign_success) {
        ...
        if ($EventType == "verifygw") {
            if ($is_sign_success) {
                $response_xml = "<success>true</success><biz_content>" . $this->getPublicKeyStr ( $this->rsaPublicKeyFilePath ) . "</biz_content>";
            } else { // echo $response_xml;
                $response_xml = "<success>false</success><error_code>VERIFY_FAILED</error_code><biz_content>" . $this->getPublicKeyStr ( $this->rsaPublicKeyFilePath ) . "</biz_content>";
            }
            $return_xml = $this->signResponse ( $response_xml, 'GBK');
            //LOG::DEBUG ( "response_xml: " . $return_xml );
            echo $return_xml;
            exit ();
        }
    }
    ...
    public function signResponse($bizContent, $charset) {
        $sign = $this->sign ($bizContent);
        $response = "<?xml version=\"1.0\" encoding=\"$charset\"?><alipay><response>$bizContent</response><sign>$sign</sign><sign_type>{$this->signType}</sign_type></alipay>";
        return $response;
    }
